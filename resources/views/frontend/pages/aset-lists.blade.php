...
@section('main-content')
		...
		@csrf
			<!-- aset Style 1 -->
			<section class="aset-area asetit-sidebar asetit-list asetit section">
				<div class="container">
					<div class="row">
										...
										<li>
											@foreach($menu as $cat_info)
													@if($cat_info->child_cat->count()>0)
														<li><a href="{{route('aset-cat',$cat_info->slug)}}">{{$cat_info->title}}</a>
															<ul>
																@foreach($cat_info->child_cat as $sub_menu)
																	<li><a href="{{route('aset-sub-cat',[$cat_info->slug,$sub_menu->slug])}}">{{$sub_menu->title}}</a></li>
																@endforeach
															</ul>
														</li>
													@else
														<li><a href="{{route('aset-cat',$cat_info->slug)}}">{{$cat_info->title}}</a></li>
													@endif
											@endforeach
										</li>
										@endif
                                        {{-- @foreach(Helper::asetCategoryList('asets') as $cat)
                                            @if($cat->is_parent==1)
												<li><a href="{{route('aset-cat',$cat->slug)}}">{{$cat->title}}</a></li>
											@endif
                                        @endforeach --}}
                                    </ul>
                                </div>
						...
						<div class="col-lg-9 col-md-8 col-12">
							...
							<div class="row">
								@if(count($asets))
											...
											<div class="row">
														...
														<div class="aset-img">
															...
															<div class="button-head">
																<div class="aset-action">
																	...
																	<a title="Wishlist" href="{{route('add-to-wishlist',$aset->slug)}}" class="wishlist" data-id="{{$aset->id}}"><i class=" ti-heart "></i><span>Add to Wishlist</span></a>
																</div>
																<div class="aset-action-2">
																	<a title="Add to cart" href="{{route('add-to-cart',$aset->slug)}}">Add to cart</a>
																</div>
															</div>
														</div>
													</div>
												...
												<div class="col-lg-8 col-md-6 col-12">
													<div class="list-content">
														...
														<a data-toggle="modal" data-target="#{{$aset->id}}" title="Quick View" href="#"class="btn cart">Pinjam Sekarang</a>
													</div>
												</div>
											</div>
										</div>
									...
									@endforeach
								@else
									<h4 class="text-warning" style="margin:100px auto;">There are no asets.</h4>
								@endif
							</div>
							 <div class="row">
                            <div class="col-md-12 justify-content-center d-flex">
                                {{-- {{$asets->appends($_GET)->links()}}  --}}
                            </div>
                          </div>
						</div>
					</div>
